...
// ─── LOW RANGE ROW MAPS (shift each clef down one octave) ─────────────────────
// Each row position keeps the same staff location but maps to a note one octave lower.
const TREBLE_ROWS_LOW = TREBLE_ROWS.map(({ note, row }) => {
  // Parse note name and octave, decrement octave by 1
  const letter = note.slice(0, -1);
  const octave = parseInt(note.slice(-1), 10);
  const lowNote = `${letter}${octave - 1}`;
  return { note: lowNote, row };
}).filter(({ note }) => NOTE_FREQ[note]); // keep only notes with known frequencies

const BASS_ROWS_LOW = BASS_ROWS.map(({ note, row }) => {
  const letter = note.slice(0, -1);
  const octave = parseInt(note.slice(-1), 10);
  const lowNote = `${letter}${octave - 1}`;
  return { note: lowNote, row };
}).filter(({ note }) => NOTE_FREQ[note]);
...
function StaffCanvas({
  ...
  rows: rowsProp,  // optional override (used for range mode)
}) {
  ...
  const rows = rowsProp ?? (isT ? TREBLE_ROWS : BASS_ROWS);
  ...
  // Debounce: ignore clicks within 350ms of the last accepted one.
  // This prevents a single physical click from registering 2–3 times due to
  // touch events, React synthetic event bubbling, or slight pointer drift.
...
function generateHeader(title, tempo, treble, bass) {
  ...
  // Always emit ALL 6 variables. Empty voices get an empty notes array so
  // startPlayback() can deserialize them without leaving uninitialized Strings.
  [['treble',treble],['bass',bass]].forEach(([clef,vs]) => {
    vs.forEach((arr,vi) => {
      const notes = arr.map(n=>({note:fmtNote(n),duration:n.duration,type:fmtType(n.type)}));
      const obj = { title:`${title} - ${clef[0].toUpperCase()+clef.slice(1)} V${vi+1}`, tempo, notes };
...
const BuzzerComposerPage = ({ setCurrentPage }) => {
  ...
  const [rangeMode, setRangeMode] = useState('high'); // 'high' | 'low'
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 1.5rem 5rem' }}>
        ...
        <div style={{ background:surface, border:`1px solid ${border}`, borderRadius:8, padding:'1rem 1.2rem', marginBottom:'1rem' }}>
          ...
          <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', alignItems:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ ...M, fontSize:'0.6rem', color:textMuted }}>RANGE</span>
              <ToggleGroup
                options={[{value:'high',label:'High'},{value:'low',label:'Low'}]}
                value={rangeMode}
                onChange={v => { setRangeMode(v); showToast(v === 'low' ? 'Low range (−1 octave)' : 'High range'); }}
              />
            </div>
        ...
        <div style={{ marginBottom:'2rem' }}>
          ...
          {treble.map((voice, vi) => (
            <div key={vi} style={{ marginBottom:'0.5rem' }}>
              ...
              <StaffCanvas
                clef="treble"
                rows={rangeMode === 'low' ? TREBLE_ROWS_LOW : TREBLE_ROWS}
                voiceNotes={voice}
        ...
        <div style={{ marginBottom:'2rem' }}>
          ...
          {bass.map((voice, vi) => (
            <div key={vi} style={{ marginBottom:'0.5rem' }}>
              ...
              <StaffCanvas
                clef="bass"
                rows={rangeMode === 'low' ? BASS_ROWS_LOW : BASS_ROWS}
                voiceNotes={voice}
