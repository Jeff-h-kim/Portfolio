...
const Header = ({ activeSection, setActiveSection, setCurrentPage, currentPage }) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const navItems = ['Home', 'About', 'Projects', 'Contact'];

  const scrollToSection = (section) => {
  const scrollAndSetActive = () => {
  ...
  if (currentPage === 'allProjects') {
    setCurrentPage('home');
    // Wait briefly to ensure the home page is rendered before scrolling
    setTimeout(scrollAndSetActive, 100);
  } else {
    scrollAndSetActive();
  }
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => scrollToSection('Home')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="51" viewBox="0 0 50 51" fill="none">
              ...
              <defs>
                <filter id="filter0_d_7_2" x="0.943399" y="19.1115" width="44.7836" height="17.2325" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
                ...
                <linearGradient id="paint0_linear_7_2" x1="2.83018" y1="24.3208" x2="50" y2="24.3208" gradientUnits="userSpaceOnUse">
                  <stop stopColor="#002A87" />
                  <stop offset="0.711538" stopColor="#572A7B" />
                </linearGradient>
                <linearGradient id="paint1_linear_7_2" x1="0" y1="26.6792" x2="47.1698" y2="26.6792" gradientUnits="userSpaceOnUse">
                  <stop stopColor="#0039B4" />
                  <stop offset="0.711538" stopColor="#743AA2" />
                </linearGradient>
              </defs>
            </svg>
          ...
          <div className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              <a
                ...
                className={`text-lg transition-all hover:text-indigo-500 hover:font-bold cursor-pointer ${activeSection === item ? 'text-indigo-500 font-bold' : 'text-gray-700'
                  }`}
              >
                {item}
              </a>
        ...
        >
          {navItems.map((item) => (
            <a
              key={item}
              onClick={() => scrollToSection(item)}
              className={`block w-full text-right text-2xl py-2 transition-colors hover:text-indigo-800 cursor-pointer ${activeSection === item ? 'text-indigo-500 font-medium' : 'text-gray-700'
                }`}
            >
              {item}
            </a>
