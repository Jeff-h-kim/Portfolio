  ...
  return (
    <header className="fixed top-0 w-full bg-white/90 backdrop-blur-md z-50 border-b !border-gray-400">
      <nav className="container mx-auto px-6 py-4">
          ...
          <div className="hidden md:flex space-x-8 items-center">
            {navItems.map((item) => (
              <a
                ...
                className={`text-lg transition-all hover:text-indigo-500 hover:font-bold cursor-pointer ${
                  activeSection === item ? 'text-indigo-500 font-bold' : 'text-gray-700'
                }`}
              >
                {item}
              </a>
            ...
            <button
              onClick={() => {
                setCurrentPage('buzzer-composer');
                setIsMenuOpen(false);
              }}
              className="text-lg text-gray-700 hover:text-indigo-500 hover:font-bold"
            >
              Composer
            </button>
          </div>
        ...
        {/* Mobile Navigation */}
        <div
          className={`md:hidden flex flex-col gap-12 overflow-hidden transition-all duration-500 ease-in-out ${
        ...
        >
          {navItems.map((item) => (
            <a
              key={item}
              onClick={() => scrollToSection(item)}
              className={`block w-full text-right text-2xl py-2 transition-colors hover:text-indigo-800 cursor-pointer ${
                activeSection === item ? 'text-indigo-500 font-medium' : 'text-gray-700'
              }`}
            >
              {item}
            </a>
          ...
          <button
            ...
            className="block w-full text-right text-2xl py-2 text-gray-700 hover:text-indigo-800"
          >
            Composer
          </button>
        </div>
